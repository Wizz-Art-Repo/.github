<div align="center">

<img src="https://img.shields.io/badge/▲_Arrow_IT-449DD7?style=for-the-badge&labelColor=3D6A8A" alt="Arrow IT" />

<br>

### Your IT Solution

**Building modern, scalable solutions from Belgium to the world.**

[![Website](https://img.shields.io/badge/arrowit.be-3D6A8A?style=for-the-badge&logo=googlechrome&logoColor=white)](https://arrowit.be)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-3D6A8A?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/company/arrow-it-srl)

---

**🇬🇧 English** · [🇫🇷 Français](#-arrow-it--fr)

</div>

<br>

## ⚡ What we do

<table>
<tr>
<td width="33%" align="center">

### 🛠️ Custom Dev

Full-stack web & mobile apps, from concept to production

</td>
<td width="33%" align="center">

### 🤖 AI Integration

Smarter workflows through intelligent automation

</td>
<td width="33%" align="center">

### 🎓 Training

Level up your team on modern frameworks & best practices

</td>
</tr>
</table>

<br>

## 💼 Invoicing by Arrow

> **SaaS invoicing built for the Belgian market** — for independents and SMEs who need simplicity without compromise.

<table>
<tr>
<td>

- ⚡ **Peppol e-invoicing** — fully compliant (mandatory in Belgium since 2026)
- 🏦 **Open Banking** — Belgian bank integrations via Ponto
- 🔍 **Auto VAT validation** — no manual checks
- 🔔 **Smart reminders** — automated payment follow-ups
- 💰 **From 9€/month** — plans that scale with you

</td>
</tr>
</table>

<div align="center">

[![Try Invoicing](https://img.shields.io/badge/Try_Invoicing_→-449DD7?style=for-the-badge&logoColor=white)](https://invoicing.arrowit.be)

</div>

<br>

## 🧰 Tech Stack

<div align="center">

![React](https://img.shields.io/badge/React-3D6A8A?style=flat-square&logo=react&logoColor=white)
![React Native](https://img.shields.io/badge/React_Native-3D6A8A?style=flat-square&logo=react&logoColor=white)
![Expo](https://img.shields.io/badge/Expo-3D6A8A?style=flat-square&logo=expo&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-449DD7?style=flat-square&logo=laravel&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-449DD7?style=flat-square&logo=nodedotjs&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-528FBF?style=flat-square&logo=mysql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-528FBF?style=flat-square&logo=docker&logoColor=white)
![Cloudflare](https://img.shields.io/badge/Cloudflare-528FBF?style=flat-square&logo=cloudflare&logoColor=white)

</div>

<br>

---

<div align="center">

<a id="-arrow-it--fr"></a>

## 🇫🇷 Arrow IT — FR

</div>

### À propos

Arrow IT est une société belge de développement logiciel spécialisée dans la création d'applications web et mobiles modernes et évolutives. Nous aidons les entreprises à digitaliser leurs flux de travail, optimiser leurs opérations et adopter l'innovation.

### ⚡ Nos Services

<table>
<tr>
<td width="33%" align="center">

### 🛠️ Dev sur mesure

Applications web & mobiles full-stack, du concept au déploiement

</td>
<td width="33%" align="center">

### 🤖 Intégration IA

Automatisation intelligente de vos workflows

</td>
<td width="33%" align="center">

### 🎓 Formation

Montée en compétences sur les frameworks modernes

</td>
</tr>
</table>

<br>

### 💼 Invoicing by Arrow

> **Facturation SaaS pour le marché belge** — pour les indépendants et PME qui veulent la simplicité sans compromis.

- ⚡ **Peppol** — conformité facturation électronique (obligatoire en Belgique depuis 2026)
- 🏦 **Open Banking** — intégrations bancaires belges via Ponto
- 🔍 **Validation TVA auto** — zéro vérification manuelle
- 🔔 **Rappels intelligents** — suivi de paiement automatisé
- 💰 **Dès 9€/mois** — des formules qui évoluent avec vous

<div align="center">

[![Découvrir](https://img.shields.io/badge/Découvrir_→-449DD7?style=for-the-badge&logoColor=white)](https://invoicing.arrowit.be)

</div>

<br>

---

<div align="center">

<sub>© 2026 Arrow IT SRL</sub>

</div>
